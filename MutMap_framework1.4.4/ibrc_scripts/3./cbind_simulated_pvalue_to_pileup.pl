...
my $pvalue_file = shift; # 1st argment # input_pvaluet_table
...
sub scan_pileup_and_cbind() {
	my($pvalue_file0, $pileup_file1, $pileup_file2) = @_;
	
	open(PVALUE0, $pvalue_file0) or die "No $pvalue_file0";
	...
	print STDERR "load p-values $pvalue_file0\n";
	while (<PVALUE0>) {
		my $oneline = $_;
		chomp $oneline;
	...
	while (<PILEUP1>) {
		...
		printf PILEUP2 "%s\t%f\t%f\t%d\t%d\t%d\t%f\t%f\t%d\t%d\t%d\t%f\t%f\t%d\t%d\t%d\n", $oneline, $p_h90{$depth}, $p_l90{$depth}, $j_l90, $j_m90, $j_h90, $p_h95{$depth}, $p_l95{$depth}, $j_l95, $j_m95, $j_h95, $p_h99{$depth}, $p_l99{$depth}, $j_l99, $j_m99, $j_h99;
...
sub usage() {
	...
	print STDERR "usage: $Command <in.pvaluetable.txt> <in.pileup> <out.pileup>\n";  
	exit; 
}
