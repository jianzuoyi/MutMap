...
#input_file open
open(INFILE1, "gunzip -c $input_file1 | ") or die "No $input_file1";
open(INFILE2, "gunzip -c $input_file2 | ") or die "No $input_file2";

#output_file open
open(OUTFILE1, " | gzip -f - > $output_file1") or die "No $output_file1";
open(OUTFILE2, " | gzip -f - > $output_file2") or die "No $output_file2";
open(OUTFILES, " | gzip -f - > $output_fileS") or die "No $output_fileS";
...
sub check_which_kind_of_id(){
	my($infastq1) = @_;
	open (my $fh1, "gunzip -c $infastq1 | ") or die "cannot open $infastq1\n";
	my $oneline1 = <$fh1>;
	chomp($oneline1);
